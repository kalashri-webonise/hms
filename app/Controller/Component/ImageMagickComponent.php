...
class ImageMagickComponent extends Component
{
     ...
     * @param bool|int $quality JPEG Quality (default: 100)
     * @param boolean $aspect Maintain aspect ratio (default: true, 1 = maintain ratio, 2 = cut image to fit)
     * @param string $image_name
     * @internal param bool $fill Maintain aspect ratio & fill (default: false)
     * @internal param array $permitted Array of HTML attributes.
     * @return string
     * @access public
     */
    function resize($filePath, $width, $height, $tag = true, $cachePath = null, $quality = 100, $aspect = true, $image_name = '01')
    {
        ...
        if ($resize) {
            ...
            exec($this->convertPath . ' ' . escapeshellarg($url) . ' -sample ' . $width . 'x' . $height . $resizeOption . ' -quality ' . $quality . $jpgOptions . ' ' . escapeshellarg($cachefile) . '');

            //            echo $this->convertPath . ' ' . escapeshellarg($url). ' -resize ' . $width . 'x' . $height . $resizeOption . ' -quality ' . $quality . $jpgOptions . ' ' . escapeshellarg($cachefile) . '';
        } else {
